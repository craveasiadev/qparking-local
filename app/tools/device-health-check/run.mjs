...
const RESULT = path.join(OUT_DIR, 'device-health.json');

rmSync(OUT_DIR, { recursive: true, force: true });
mkdirSync(OUT_DIR, { recursive: true });

await runUnderElectron(path.join('tools', 'device-health-check', 'check.js'), [RESULT]);
...
console.log(`\n${failed === 0 ? `PASS — ${r.checks.length} device-health checks green` : `FAIL — ${failed} check(s) failed`}`);
