...
| **E · every pass type** | all nine `App\Enum\SeasonPass\PassType` values — monthly, quarterly, yearly, staff, corporate, free_access, vip, temporary, resident — each proving a free exit on a charging plan with `freeReason = pass-<type>` and the `pass_id` in the audit row |
