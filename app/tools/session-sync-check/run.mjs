/**
 * Session → cloud delivery harness runner.  npm run test:session-sync
 *
...
const RESULT = path.join(OUT_DIR, 'session-sync.json');
...
await runUnderElectron(path.join('tools', 'session-sync-check', 'check.js'), [RESULT]);
...
console.log(`\n${failed === 0 ? `PASS — ${r.checks.length} session-sync checks green` : `FAIL — ${failed} check(s) failed`}`);
