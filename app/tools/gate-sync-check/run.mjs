...
const RESULT = path.join(OUT_DIR, 'gate-sync.json');

rmSync(OUT_DIR, { recursive: true, force: true });
mkdirSync(OUT_DIR, { recursive: true });

await runUnderElectron(path.join('tools', 'gate-sync-check', 'check.js'), [RESULT]);
...
console.log(`\n${failed === 0 ? `PASS — ${r.checks.length} gate-sync checks green` : `FAIL — ${failed} check(s) failed`}`);
